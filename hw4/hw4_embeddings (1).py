...
def indexation(corpus):
    ...
    bert_X = np.load('./drive/MyDrive/bert.npy')
...
def main():
    corpus, lemmas = build_corpus('./MyDrive/questions_about_love.jsonl')
    bert_matrix, matrix = indexation(lemmas)
    query = input('Введите свой запрос: ')
    while query != '':
        model = inpit('По умолчанию используется FastText. Введите "BERT", если хотите использовать его: ')
        if model.lower() == 'bert':
...
def main():
    corpus, lemmas = build_corpus('./drive/MyDrive/questions_about_love.jsonl')
    bert_matrix, matrix = indexation(lemmas)
    query = input('Введите свой запрос: ')
    while query != '':
